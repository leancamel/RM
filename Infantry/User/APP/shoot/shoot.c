...
{
    //上拨判断， 一次开启，再次关闭
    if (switch_is_fric_on(shoot_control.shoot_rc->rc.ch[4]))
    {
    ...
    {
        //下拨一次或者鼠标按下一次，进入射击状态
        if (switch_is_shoot(shoot_control.shoot_rc->rc.ch[4]) || (shoot_control.press_l && shoot_control.last_press_l == 0) || (shoot_control.press_r && shoot_control.last_press_r == 0))
        {
